...
export default function MapelIndex({ auth, mapel_list, guru_list = [], filters = {} }) {
    const [searchTerm, setSearchTerm] = useState(filters.search || '');
    const [selectedGuru, setSelectedGuru] = useState(filters.guru_id || '');
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => {
            router.get(route('admin.mapel.index'), { search: searchTerm, guru_id: selectedGuru }, { preserveState: true, replace: true });
        }, 300);
        return () => clearTimeout(t);
    }, [searchTerm]);

    const handleGuruChange = (v) => {
        setSelectedGuru(v);
        router.get(route('admin.mapel.index'), { search: searchTerm, guru_id: v }, { preserveState: true, replace: true });
    };
    const handleReset = () => {
        setSearchTerm(''); setSelectedGuru('');
        router.get(route('admin.mapel.index'), {}, { preserveState: true, replace: true });
    };
    const hapus = (id, nama) => { if (confirm(`Hapus mapel "${nama}"?`)) router.delete(route('admin.mapel.destroy', id)); };

    const exportPdf   = () => { window.location.href = route('admin.mapel.export.pdf'); };
    const exportExcel = () => { window.location.href = route('admin.mapel.export.excel'); };
    ...
    const handleImportSubmit = (e) => {
        ...
        router.post(route('admin.mapel.import'), data, {
            forceFormData: true,
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                        <div className="bg-primary px-6 py-4 flex items-center justify-between">
                            <div>
                                <h3 className="text-white font-bold text-base">Impor Data Mapel</h3>
                        ...
                        <form onSubmit={handleImportSubmit} className="p-6 space-y-4">
                            <div
                                ...
                                onDrop={handleDrop}
                                onClick={() => document.getElementById('importFileMapel').click()}
                                ...
                                <p className="text-xs text-gray-400 mt-1">Kolom: Kode Mapel | Nama Mapel | Nama Guru</p>
                                <input id="importFileMapel" type="file" accept=".xlsx,.xls,.csv" className="hidden"
                            ...
                            {importFile && (
                                <div className="flex items-center gap-2 px-3 py-2 bg-emerald-50 border border-emerald-100 rounded-lg">
                                    ...
                                    <button type="button" onClick={() => setImportFile(null)} className="text-red-400 hover:text-red-600">
            ...
            <section className="px-5 pb-5">
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-6 border-b border-gray-100">
                        <div>
                            <h3 className="text-lg font-bold text-text">Daftar Mata Pelajaran</h3>
                            <p className="text-xs text-muted font-bold uppercase tracking-wider mt-0.5">Kelola Mata Pelajaran &amp; Penugasan Guru</p>
                        </div>
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                            ...
                            <select value={selectedGuru} onChange={(e) => handleGuruChange(e.target.value)}
                        ...
                        <table className="w-full text-sm min-w-[600px] border-collapse">
                            <thead>
                                <tr className="bg-slate-50/70 border-b border-gray-100">
                                    <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase text-left tracking-wider">Kode</th>
                                    <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase text-left tracking-wider">Nama Mapel</th>
                                    <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase text-left tracking-wider hidden sm:table-cell">Guru Pengampu</th>
                                    <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase text-center tracking-wider">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                {mapel_list.map((m) => (
                                    <tr key={m.id} className="border-b border-gray-50 last:border-0 hover:bg-slate-50/50 transition-colors">
                                        <td className="px-6 py-4 font-mono text-xs font-bold text-primary">{m.kode_mapel}</td>
                                        <td className="px-6 py-4 font-bold text-text">{m.nama_mapel}</td>
                                        <td className="px-6 py-4 text-gray-500 text-xs font-semibold hidden sm:table-cell">{m.guru}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center justify-center gap-3">
                                                <Link href={route('admin.mapel.edit', m.id)} className="text-xs font-bold text-blue-600 hover:text-blue-800 transition flex items-center gap-1">
                                                    <i className="fa-solid fa-pen-to-square" /> Edit
                                                </Link>
                                                <button onClick={() => hapus(m.id, m.nama_mapel)} className="text-xs font-bold text-red-500 hover:text-red-700 transition flex items-center gap-1">
                                ...
                                {mapel_list.length === 0 && (
                                    <tr><td colSpan={4} className="px-6 py-12 text-center text-gray-400 font-medium">Belum ada mata pelajaran</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
