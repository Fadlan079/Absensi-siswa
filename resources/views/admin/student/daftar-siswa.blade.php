...
@forelse ($siswa as $namaKelas => $daftarJurusan)
<section class="p-5">

    <div class="border-l-[5px] border-secondary rounded-lg bg-secondary/10 py-3 px-4 mb-4">
        <h2 class="font-black text-text text-xl tracking-wide uppercase">Kelas: {{ $namaKelas }}</h2>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        @foreach ($daftarJurusan as $namaJurusan => $listSiswa)
            ...
            <div>
                <div class="bg-gray-50/50 px-4 py-3 border-b border-gray-100 flex justify-between items-center">
                    <h3 class="font-bold text-text text-sm md:text-base tracking-tight text-primary">
                        <i class="fa-solid fa-graduation-cap mr-1.5 text-secondary"></i> {{ $namaJurusan }}
                    </h3>
                    <span class="bg-primary text-white text-xs px-2 py-0.5 rounded-full font-semibold shrink-0">
                        {{ $listSiswa->count() }} Siswa
                    </span>
                </div>
                            ...
                            <tr class="hover:bg-gray-50/40 transition">
                                ...
                                <td class="py-2 px-4 text-center">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        <a href="{{ route('students.edit', $s->id) }}" class="p-1.5 bg-primary/10 hover:bg-primary/20 text-primary rounded transition" title="Edit Siswa">
...
@empty
<section class="p-5">
    <div class="bg-white rounded-lg shadow-md border-t-[5px] border-primary p-12 text-center text-gray-400">
        <div class="flex flex-col items-center justify-center gap-2">
            <i class="fa-solid fa-folder-open text-4xl text-gray-300"></i>
            <span class="font-medium">Belum ada data siswa yang dikelompokkan.</span>
        </div>
    </div>
</section>
@endforelse
